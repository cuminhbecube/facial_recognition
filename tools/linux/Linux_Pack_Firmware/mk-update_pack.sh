...
{
	LAYOUT_V2='256K(env),1M@256K(idblock),1M(uboot),5M(boot),208M(rootfs),32M(gshtdata),8960K(dashcfg)'
	LAYOUT_FACIAL_RECOGNITION='256K(env),256K@256K(idblock),512K(uboot),4M(boot),60M(oem),10M(userdata),180M(rootfs)'
	if ! strings "$IMAGE_PATH/$PARTITION_FILE" | grep -Fq "$LAYOUT_V2" && \
		! strings "$IMAGE_PATH/$PARTITION_FILE" | grep -Fq "$LAYOUT_FACIAL_RECOGNITION"; then
		msg_error "Refusing package: env partition map is not an approved RV06 layout"
		exit -1
	fi
	IFS=,
	for part in $SPECIAL_PARTITIONS $PARTITIONS;
	do
		for persistent in $PERSISTENT_PARTITIONS;
		do
			if [ "$part" = "$persistent" ]; then
				msg_info "preserve persistent partition: $part"
				continue 2
			fi
		done
		case $part in
...
PACKAGE_CFG=""
ENV_ALL_OPTS=""
PERSISTENT_PARTITIONS="dashcfg,gshtdata"
RV06_FACTORY_LAYOUT_FLASH=${RV06_FACTORY_LAYOUT_FLASH:-0}
