...
# Clean PATH to remove spaces, dots, and empty/relative directories for buildroot compatibility
export PATH=$(echo "$PATH" | tr ':' '\n' | grep -v '^\.$' | grep -v '^$' | grep -v ' ' | tr '\n' ':' | sed 's/:$//')
[ -d "/tmp/host-gcc-12" ] && export PATH="/tmp/host-gcc-12:$PATH"
...
function build_app() {
	...
	# A dedicated product image must not inherit packages staged by another app.
	if [ "${RK_FACIAL_RECOGNITION_APP:-n}" = "y" ]; then
...
function verify_rv06_app_stage() {
	local source_bin staged_bin source_sha staged_sha
	# The RV06 dashcam artifact check is only applicable to the legacy dashcam
	# product.  The facial-recognition profile deliberately stages a different
	# payload under /oem.
	[ "${RK_FACIAL_RECOGNITION_APP:-n}" = "y" ] && return 0
	source_bin="$SDK_APP_DIR/dashcam/rv06_dashcam"
	staged_bin="$RK_PROJECT_PATH_APP/bin/rv06_dashcam"

	if [ ! -x "$source_bin" ] || [ ! -x "$staged_bin" ]; then
		msg_error "RV06 app artifact missing: source=$source_bin staged=$staged_bin"
		exit 1
	fi
	if ! cmp -s "$source_bin" "$staged_bin"; then
		source_sha=$(sha256sum "$source_bin" | awk '{print $1}')
		staged_sha=$(sha256sum "$staged_bin" | awk '{print $1}')
		msg_error "RV06 app staging mismatch: source_sha=$source_sha staged_sha=$staged_sha"
		exit 1
	fi
	msg_info "RV06 app stage verified: sha256=$(sha256sum "$staged_bin" | awk '{print $1}')"
}

function verify_rv06_rootfs_freshness() {
	local rootfs_tarball overlay_root stale_file
	rootfs_tarball="$RK_PROJECT_PATH_SYSDRV/rootfs_${RK_LIBC_TPYE}_${RK_CHIP}.tar"
	overlay_root="$SDK_SYSDRV_DIR/tools/board/buildroot/overlay"

	[ -d "$overlay_root" ] || return 0
	[ -f "$rootfs_tarball" ] || return 0
	stale_file=$(find "$overlay_root" -type f -newer "$rootfs_tarball" -print -quit)
	if [ -n "$stale_file" ]; then
		msg_error "Rootfs input is newer than packaged tarball: $stale_file"
		msg_error "Run './build.sh rootfs' before packaging firmware. Refusing stale rootfs."
		exit 1
	fi
}

function verify_rv06_oem_stage() {
	local staged_bin packaged_bin
	[ "${RK_FACIAL_RECOGNITION_APP:-n}" = "y" ] && return 0
	staged_bin="$RK_PROJECT_PATH_APP/bin/rv06_dashcam"
	packaged_bin="$RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/rv06_dashcam"

	if [ ! -x "$packaged_bin" ] || ! cmp -s "$staged_bin" "$packaged_bin"; then
		msg_error "RV06 OEM package does not contain the verified app artifact: $packaged_bin"
		exit 1
	fi
	msg_info "RV06 OEM artifact verified: sha256=$(sha256sum "$packaged_bin" | awk '{print $1}')"
}
...
function build_driver(){
	...
	if [ "${RK_FACIAL_RECOGNITION_APP:-n}" = "y" ]; then
		...
		mkdir -p "$RK_PROJECT_PATH_SYSDRV/kernel_drv_ko"
		cp -a "$SDK_SYSDRV_DIR/out/kernel_drv_ko/." "$RK_PROJECT_PATH_SYSDRV/kernel_drv_ko/"
	fi
...
function build_updateimg(){
	...
	# Publish a versioned image only after Rockchip packaging succeeds.
	sh "$PROJECT_TOP_DIR/scripts/export_upgrade.sh" \
		"$SDK_ROOT_DIR" "$IMAGE_PATH/update.img"
...
{
	# Do not let packages from another product (notably dashcam Wi-Fi modules)
	# leak into the dedicated facial-recognition OEM image.
	if [ "${RK_FACIAL_RECOGNITION_APP:-n}" = "y" ]; then
...
function build_firmware(){
	...
	# Firmware packaging must never consume a stale app_out tree.  Rebuild/copy
	# the app first, then verify the exact bytes that will enter the image.
	build_app
	verify_rv06_app_stage
	verify_rv06_rootfs_freshness
	...
	__PACKAGE_ROOTFS
	# This product is Ethernet-only.  The shared RV06 rootfs overlay carries
	# dashcam AP/WPA startup files, which must not be shipped in this image.
	if [ "${RK_FACIAL_RECOGNITION_APP:-n}" = "y" ]; then
		rm -f "$RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S50hostapd" \
			"$RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S70rv06-wifi-ap" \
			"$RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S80dnsmasq" \
			"$RK_PROJECT_PACKAGE_ROOTFS_DIR/lib/udev/rules.d/90-wlan0-wpa.rules"
	fi
	__PACKAGE_OEM
	verify_rv06_oem_stage
