...
function mk_ubi_image_fake()
{
	...
	if [ $(( $DEFAULT_UBI_PAGE_SIZE )) -eq $(( $UBI_PAGE_SIZE )) \
		...
		# Some Rockchip upgrade tools reject a symlink even when its target exists.
		echo "cp -f --remove-destination $output_image $IMAGE_OUTPUT_DIR/${UBI_VOL_NAME}.img" >> $UBI_IMAGE_FAKEROOT
	fi
