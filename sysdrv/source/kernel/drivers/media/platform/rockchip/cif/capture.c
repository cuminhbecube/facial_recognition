...
static int rkcif_csi_stream_start(struct rkcif_stream *stream, unsigned int mode)
{
	...
	v4l2_info(&dev->v4l2_dev,
		  "[RKCIF_DBG] stream=%u active_sensor=%px sensor_sd=%px terminal_sensor=%px cif_fmt_in=%px cif_fmt_out=%px dphy=%px\n",
		  stream->id, active_sensor,
		  active_sensor ? active_sensor->sd : NULL,
		  dev->terminal_sensor.sd, stream->cif_fmt_in,
		  stream->cif_fmt_out, dev->dphy_hw);
	if (!active_sensor || !stream->cif_fmt_in || !stream->cif_fmt_out) {
		v4l2_err(&dev->v4l2_dev,
			 "[RKCIF_DBG] stream=%u invalid capture state: active_sensor=%px cif_fmt_in=%px cif_fmt_out=%px\n",
			 stream->id, active_sensor, stream->cif_fmt_in,
			 stream->cif_fmt_out);
		return -ENODEV;
	}

	flags = active_sensor->mbus.flags;
	mbus_type = active_sensor->mbus.type;
...
void rkcif_set_default_fmt(struct rkcif_device *cif_dev)
{
	...
	if (cif_dev->terminal_sensor.sd) {
		for (i = 0; i < stream_num; i++) {
			...
			if (ret) {
				v4l2_err(&cif_dev->v4l2_dev,
					 "set default stream[%d] get_fmt failed: %d\n",
					 i, ret);
				continue;
			}
			...
			ret = rkcif_set_fmt(&cif_dev->stream[i], &pixm, false);
			if (ret)
				v4l2_err(&cif_dev->v4l2_dev,
					 "set default stream[%d] mbus=0x%x fourcc=0x%x failed: %d\n",
					 i, fmt.format.code, pixm.pixelformat, ret);
		}
	}
}
