...
#define SC235HAI_GAIN_MIN		0x40
#define SC235HAI_GAIN_MAX		(117 * 16 * 64)       // 116.55*15.875*32
...
static int sc235hai_set_gain_reg(struct sc235hai *sc235hai, u32 total_gain, int mode)
{
	...
	u32 again_num[9]={1000,2000,3700,7400,14800,29600,59200,116550};
	u32 again_reg[7]={0x00,0x01,0x80,0x81,0x83,0x87,0x8f};
	u32 dgain_reg[4]={0x00,0x01,0x03,0x07};
    u32 gain_num[5]={116550,116550*2,116550*4,116550*8,116550*16};

	if (total_gain < SC235HAI_GAIN_MIN)
		total_gain = SC235HAI_GAIN_MIN;
	else if (total_gain > SC235HAI_GAIN_MAX)
		total_gain = SC235HAI_GAIN_MAX;

	gain_factor = total_gain * 1000 / 64;

    if((gain_factor/116550) >0 ){
        fine_again = 0x3f;
        coarse_again=0x8f;

        for(i=0;i<4;i++){
            if(gain_factor < gain_num[i+1]){
                coarse_dgain=dgain_reg[i];
                fine_dgain=gain_factor*128/gain_num[i];
                break;
            }
        }
    }else{
        for(i=0;i<9;i++){
            if(gain_factor < again_num[i+1]){
                coarse_again = again_reg[i];
                coarse_dgain = 0x00;
                fine_dgain = 0x80;
                fine_again = gain_factor * 32 / again_num[i];
                break;
            }
        }
    }
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		*v4l2_subdev_get_try_format(sd, cfg, fmt->pad) = fmt->format;
		...
		__v4l2_ctrl_s_ctrl_int64(sc235hai->pixel_rate, pixel_rate);
				  ...
				  struct v4l2_subdev_mbus_code_enum *code)
{
	...
	if (code->index != 0)
		return -EINVAL;
	code->code = sc235hai->cur_mode->bus_fmt;
...
static long sc235hai_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_HDR_CFG:
		...
		if (i == ARRAY_SIZE(supported_modes)) {
			...
			w = sc235hai->cur_mode->hts_def - sc235hai->cur_mode->width;
			h = sc235hai->cur_mode->vts_def - sc235hai->cur_mode->height;
			__v4l2_ctrl_modify_range(sc235hai->hblank, w, w, 1, w);
			__v4l2_ctrl_modify_range(sc235hai->vblank, h,
						 SC235HAI_VTS_MAX - sc235hai->cur_mode->height,
						 1, h);
...
static const struct dev_pm_ops sc235hai_pm_ops = {
	SET_RUNTIME_PM_OPS(sc235hai_runtime_suspend,
			   sc235hai_runtime_resume, NULL)
...
static void sc235hai_modify_fps_info(struct sc235hai *sc235hai)
{
	...
	sc235hai->cur_fps.denominator = mode->max_fps.denominator * mode->vts_def /
				       sc235hai->cur_vts;
}
				   ...
				   struct i2c_client *client)
{
	...
	dev_info(dev, "Detected SC235HAI (%06x) sensor\n", CHIP_ID);
